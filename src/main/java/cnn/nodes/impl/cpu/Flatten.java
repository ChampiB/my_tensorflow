...
public class Flatten extends Node {
    ...
    public ArrayPtr activation(ArrayPtr x) {
        ...
        for (int i = 1; i < shape.length; i++)
            res *= shape[i];
        return ArrayPtrFactory.fromData(x.toCPU().reshape(bs, res));
    }
    ...
    public ArrayPtr update(ArrayPtr gradient) {
        return ArrayPtrFactory.fromData((shape == null) ? gradient.toCPU().reshape() : gradient.toCPU().reshape(shape));
    }
     ...
     */
    public void print() {
        System.out.println("Type: Flatten (cpu)");
    }
}

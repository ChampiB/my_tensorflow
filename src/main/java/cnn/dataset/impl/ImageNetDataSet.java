...
public class ImageNetDataSet extends DataSet {
    ...
    private class Iterator {
         ...
         * @return the image.
         */
        private INDArray loadImage(File[] files) {
            INDArray result = null;
            while (result == null) {
                try {
                    result = loader.asMatrix(files[imagesIdx]);
                } catch (IOException e) {
                    incrIndexes();
                }
            }
        ...
        private void nextBatch() {
            ...
            for (int i = 0; i < inputShape[0]; i++) {
                try {
                    File[] files = Objects.requireNonNull(new File(labelsDir[labelsIdx]).listFiles(File::isFile));
                    INDArray image = loadImage(files);
                    features.toCPU().put(
                            new INDArrayIndex[] {
                    ...
                    for (int j = 0; j < labels.getShape()[1]; j++) {
    ...
    public ImageNetDataSet(int batchSize) {
        this(new int[]{batchSize, 3, 224, 224}, "/cluster/home/arc/tmac2/imagenet", "/cluster/home/arc/tmac2/imagenet");
    }
